...
export async function POST(req: NextRequest) {
  ...
  const { error } = await resend.emails.send({
    ...
    replyTo: email,
    subject: `Message from ${esc(name)}: ${esc(subject)}`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a3c3e;">
        <div style="background:#1F6F7A;padding:24px 32px;border-radius:8px 8px 0 0;">
          ...
          <p style="margin:4px 0 0;color:#b2d8db;font-size:13px;">Western Lodge — received via contact form</p>
        </div>
        <div style="background:#fff;padding:32px;border:1px solid #e2d9c8;border-top:none;border-radius:0 0 8px 8px;">
          <table style="width:100%;border-collapse:collapse;font-size:14px;">
            <tr style="border-bottom:1px solid #f0ece4;">
              <td style="padding:10px 0;font-weight:600;width:80px;color:#5a7a7c;">From</td>
            ...
            <tr style="border-bottom:1px solid #f0ece4;">
              <td style="padding:10px 0;font-weight:600;color:#5a7a7c;">Subject</td>
              <td style="padding:10px 0;">${esc(subject)}</td>
            </tr>
            <tr>
              <td style="padding:10px 0;font-weight:600;color:#5a7a7c;vertical-align:top;">Message</td>
              <td style="padding:10px 0;white-space:pre-line;">${esc(message)}</td>
            </tr>
          </table>
