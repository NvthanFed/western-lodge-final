...
export function ContactSection() {
  ...
  const inputClass =
    "w-full font-body text-sm bg-white border border-sand-300 text-teal-900 placeholder-teal-900/35 rounded-xl px-4 py-3 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/15 transition-all duration-200";
    ...
    <section id="contact" className="bg-ivory section-y">
      <div className="max-w-7xl mx-auto section-x">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
                ...
                return (
                  <div key={item.label} className="flex gap-4">
                    <div className="w-10 h-10 bg-teal-500/10 rounded-xl flex items-center justify-center shrink-0">
                      {Icon && <Icon className="w-4.5 h-4.5 text-teal-500" strokeWidth={1.5} />}
                    </div>
                    <div>
                      <p className="font-body font-600 text-[0.65rem] tracking-[0.2em] uppercase text-teal-500 mb-1">{item.label}</p>
                      {item.href ? (
                        <a href={item.href} className="font-body text-sm text-teal-900 hover:text-teal-500 transition-colors leading-relaxed whitespace-pre-line break-all">
                          {item.value}
                        </a>
                      ) : (
                        <p className="font-body text-sm text-teal-900 leading-relaxed whitespace-pre-line">{item.value}</p>
                      )}
                    </div>
                  </div>
                );
          ...
          {/* Right — contact form */}
          <FadeInUp delay={0.12}>
            <div className="bg-sand-200 rounded-2xl p-6 md:p-8 border border-sand-300/70">
              ...
              {status === "success" ? (
                <div className="flex flex-col items-center text-center py-8">
                  <motion.div className="w-16 h-16 mb-4">
                    <svg viewBox="0 0 64 64" fill="none" className="w-full h-full">
                      <motion.circle cx="32" cy="32" r="28" stroke="#4A7C59" strokeWidth="3"
                        initial={{ pathLength: 0, opacity: 0 }}
                        animate={{ pathLength: 1, opacity: 1 }}
                        transition={{ duration: 0.5, ease: "easeOut" }} />
                      <motion.path d="M18 32 L28 42 L46 22" stroke="#4A7C59" strokeWidth="3"
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 font-body font-600 text-sm tracking-wide bg-teal-500 hover:bg-teal-600 disabled:bg-teal-300 text-white py-3.5 rounded-xl transition-all duration-300"
                  >
                    {status === "submitting" ? (
